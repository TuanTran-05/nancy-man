...
3. [ ] Provision `/etc/edutrack-ops/{web,collector}.env`, `OPS_DATA_KEY`, PostgreSQL URL, Zalo token and the approved non-empty `OPS_ALERT_ZALO_RECIPIENT_UIDS` allowlist outside git; verify modes and ownership.
...
10. [ ] Send a synthetic critical event, then two healthy samples; verify exactly one approved-recipient Zalo critical alert and one recovery message with no PII, secret or excerpt.
