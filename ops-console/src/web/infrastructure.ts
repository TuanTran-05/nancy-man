import type { DashboardOverview, InfrastructureHistoryPoint, MonitorSample } from '../shared/models.js';

const unavailable = 'Không khả dụng';
const numberFormat = new Intl.NumberFormat('vi-VN', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
const bytesFormat = new Intl.NumberFormat('vi-VN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const loadFormat = new Intl.NumberFormat('vi-VN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
export function isInfrastructureUnavailable(overview: DashboardOverview, now = Date.now()): boolean {
  ...
  if (!beszel || beszel.level !== 'healthy' || beszel.details.probeOk !== true || !resources) return true;
  const observedAt = Date.parse(resources.observedAt);
  return !Number.isFinite(observedAt) || now - observedAt > 150_000;
}

export type ChartMetric = keyof Pick<InfrastructureHistoryPoint, 'cpuPercent' | 'memoryPercent' | 'diskPercent' | 'load1' | 'networkReceiveBytesPerSecond' | 'networkTransmitBytesPerSecond' | 'diskReadBytesPerSecond' | 'diskWriteBytesPerSecond'>;
export interface ChartPoint { x: number; y: number; value: number; }

export function makeLineSegments(points: InfrastructureHistoryPoint[], metric: ChartMetric, width: number, height: number): ChartPoint[][] {
  const values = points.map((point) => point[metric]).filter(validNumber);
  if (!values.length || width <= 0 || height <= 0) return [];
  const min = Math.min(...values);
  const max = Math.max(...values);
  const span = max - min;
  const xFor = (index: number): number => points.length <= 1 ? width / 2 : (index / (points.length - 1)) * width;
  const yFor = (value: number): number => span === 0 ? height / 2 : height - ((value - min) / span) * height;
