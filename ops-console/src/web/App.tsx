...
import { acknowledgeIncident, getOverview, getSession, getZaloLink, login, logout, type SessionInfo, type ZaloLinkInfo } from './api.js';
...
const monitorOrder = ['app_liveness', 'app_health', 'app_process', 'postgres', 'cron', 'backup', 'errors', 'collector', 'beszel', 'host_resources', 'host_services'] as const;
...
export function App() {
  ...
  const refresh = useCallback(async () => { try { const [nextOverview, nextLink] = await Promise.all([getOverview(), getZaloLink()]); setOverview(nextOverview); setZaloLink(nextLink); } catch (error) { if (String(error).includes('HTTP_401')) setSession(null); } }, []);
  ...
  return <main className="app-shell"><header className="app-header"><div><p className="eyebrow">THIEN UY / OPS</p><h1>Ops Console</h1></div><div className="header-actions"><span className="muted">{session.username}</span><button type="button" onClick={() => logout(session.csrfToken).then(() => setSession(null)).catch(() => setSession(null))}>Đăng xuất</button></div></header><OverviewCards overview={overview} /><InfrastructureSection overview={overview} />{zaloLink ? <ZaloLinkPanel info={zaloLink} csrfToken={session.csrfToken} onChanged={refresh} /> : null}<section><h2>Monitor</h2><div className="monitor-grid">{monitorOrder.map((monitor) => <MonitorPanel key={monitor} monitor={monitor} sample={overview.latestByMonitor[monitor]} />)}</div></section><IncidentList incidents={overview.openIncidents} onAcknowledge={setSelectedIncident} /><p className="footer-note">Console chỉ hiển thị trạng thái vận hành của hệ thống.</p>{selectedIncident ? <AcknowledgeDialog incident={selectedIncident} onClose={() => setSelectedIncident(null)} onSubmit={async (note) => { await acknowledgeIncident(selectedIncident.id, note, session.csrfToken); setSelectedIncident(null); await refresh(); }} /> : null}</main>;
}
