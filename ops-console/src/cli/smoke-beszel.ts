...
import { BeszelClientError, createBeszelClient, type BeszelRawSnapshot } from '../server/beszel/client.js';
...
export async function smokeBeszelContract(config: CollectorConfig, reader?: BeszelSnapshotReader, now = new Date()): Promise<BeszelSmokeResult> {
  ...
  const metricAgeSeconds = Math.max(0, Math.floor((now.getTime() - Date.parse(normalized.metricObservedAt)) / 1000));
...
if (process.argv[1]?.endsWith('/smoke-beszel.js')) {
  Promise.resolve().then(() => smokeBeszelContract(loadCollectorConfig(process.env))).then((result) => {
    process.stdout.write(`${JSON.stringify(result)}\n`);
  }).catch((error: unknown) => {
    const code = error instanceof BeszelClientError || error instanceof Error ? error.message : 'beszel_smoke_failed';
