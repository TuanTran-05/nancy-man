...
const databaseUrl = process.env.OPS_MONITOR_DATABASE_URL;

describe.skipIf(!databaseUrl)('ops_monitor PostgreSQL contract', () => {
  it('can execute the aggregate but has no public DDL/DML capability', async () => {
    ...
    try {
      ...
      `);
      expect(privileges.rows[0]).toMatchObject({ can_create_database: false, can_create_public: false, unexpected_membership: false });
      await expect(client.query('CREATE TABLE public.ops_monitor_contract_forbidden (id integer)')).rejects.toThrow();
    } finally {
      await client.end();
    }
  });
