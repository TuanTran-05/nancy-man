  ...
  for (const directory of directories) {
    try {
      for (const entry of readdirSync(directory, { withFileTypes: true })) {
        ...
        files.push({ name: entry.name, mtimeMs: stat.mtimeMs, size: stat.size, encrypted: /\.(?:enc|gpg|age)$/u.test(entry.name), checksumPresent });
      }
    } catch {
...
export function parseCronAndBackupState(input: CronBackupInput, now: Date = new Date()): ParsedCronBackupState {
  ...
  if (!latest || ageHours !== null && ageHours > 26) { backupLevel = 'critical'; backupErrorCode = 'backup_stale'; }
  else if (!latest.encrypted || !latest.checksumPresent) { backupLevel = 'critical'; backupErrorCode = 'backup_unverified'; }
  else if (diskUsagePercent !== null && diskUsagePercent > 90) { backupLevel = 'critical'; backupErrorCode = 'backup_disk_critical'; }
  else if (diskUsagePercent !== null && diskUsagePercent > 80) { backupLevel = 'warning'; backupErrorCode = 'backup_disk_warning'; }
  else if (latest.encrypted) { backupLevel = 'warning'; backupErrorCode = 'backup_local_only'; }
  return {
    cron,
