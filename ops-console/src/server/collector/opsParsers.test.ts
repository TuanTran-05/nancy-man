...
describe('cron and backup parser', () => {
  ...
  it('keeps backup local-only warning separate from a healthy cron monitor', () => {
    ...
    try {
      const state = parseCronAndBackupState({ cronLines: [], backupDir: directory }, new Date('2026-08-23T01:00:00Z'));
      expect(state).toMatchObject({ cronLevel: 'healthy', cronErrorCode: null, backupLevel: 'warning', backupErrorCode: 'backup_local_only' });
    } finally {
  ...
  it('reports cron failure independently of a valid backup warning', () => {
    const state = parseCronAndBackupState({
      ...
      backupFiles: [{ name: 'db.age', mtimeMs: Date.parse('2026-08-23T00:00:00Z'), size: 1, encrypted: true, checksumPresent: true }],
    }, new Date('2026-08-23T01:00:00Z'));
    expect(state).toMatchObject({ cronLevel: 'critical', cronErrorCode: 'cron_failure', backupLevel: 'warning', backupErrorCode: 'backup_local_only' });
  });
});
