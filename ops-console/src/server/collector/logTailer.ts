...
export function tailSinceCursor(path: string, cursor?: FileCursor): TailResult {
  ...
  let offset = rotated || (cursor && cursor.offset > size) ? (startAtEnd ? size : 0) : cursor.offset;
  if (offset < 0) offset = 0;
  if (offset >= size) return { lines: [], cursor: { inode, offset: size }, startedAtEnd: startAtEnd };
  ...
  const consumedBytes = Buffer.byteLength(completeLines.length ? `${completeLines.join('\n')}\n` : '', 'utf8');
  return { lines: completeLines, cursor: { inode, offset: offset + consumedBytes }, startedAtEnd: false };
}
