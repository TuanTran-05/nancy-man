...
export async function probePostgres(config: PostgresProbeConfig, now: Date = new Date()): Promise<MonitorSample> {
  const started = performance.now();
  let client: ProbeClient | undefined;
  try {
    client = config.clientFactory
      ? await config.clientFactory()
      : (new Client({ connectionString: config.postgresUrl, connectionTimeoutMillis: config.timeoutMs ?? 5000, query_timeout: config.timeoutMs ?? 5000 }) as unknown as ProbeClient);
