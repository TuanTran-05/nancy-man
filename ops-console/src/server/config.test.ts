...
describe('ops configuration', () => {
  ...
  it('validates the enabled Beszel configuration fail-closed', () => {
    ...
    try {
      expect(loadCollectorConfig(enabledEnv).beszel).toMatchObject({ enabled: true, timeoutMs: 5000, systemId: 'abc123def456ghi' });
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_URL: 'http://127.0.0.1:8080' })).toThrow('OPS_BESZEL_URL');
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_URL: 'http://127.0.0.1:8090/path' })).toThrow('OPS_BESZEL_URL');
      ...
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_TIMEOUT_MS: '999' })).toThrow('between 1000 and 10000');
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_TIMEOUT_MS: '10001' })).toThrow('between 1000 and 10000');
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_USER: 'not-an-email' })).toThrow();
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_SYSTEM_ID: 'ABC' })).toThrow();
      expect(() => loadCollectorConfig({ ...enabledEnv, OPS_BESZEL_ENABLED: 'yes' })).toThrow('true or false');
