...
const transition = (overrides: Partial<CollectorTransition> = {}): CollectorTransition => ({
  ...
  sample: { monitor: 'app_liveness', level: 'critical', observedAt: '2026-08-23T00:00:00Z', latencyMs: null, details: {}, errorCode: 'app_down' },
  level: 'critical',
  transition: 'opened',
...
describe('alert outbox', () => {
  it('sends on transition, suppresses the same fingerprint for 30 minutes, then sends recovery once', async () => {
    ...
    const store = createOpsStore(join(directory, 'ops.sqlite'), () => new Date('2026-08-23T00:00:00Z'));
    const sender = vi.fn(async () => ({ messageId: '1' }));
    const service = createAlertService({ store, botToken: 'secret', recipientIds: ['ops-a'], timeoutMs: 5000, now: () => new Date('2026-08-23T00:00:00Z'), sender });
    try {
      ...
      const recovered = transition({ transition: 'recovered', sample: { ...transition().sample, level: 'healthy', errorCode: null }, occurrenceCount: 3 });
  ...
  it('never places a redacted excerpt in a Zalo message', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'ops-alert-'));
    const store = createOpsStore(join(directory, 'ops.sqlite'), () => new Date('2026-08-23T00:00:00Z'));
    const sender = vi.fn(async (_config, text: string) => { expect(text).not.toContain('Bearer'); return { messageId: '1' }; });
    const service = createAlertService({ store, botToken: 'secret', recipientIds: ['ops-a'], timeoutMs: 5000, now: () => new Date('2026-08-23T00:00:00Z'), sender });
    try { await service.queueTransitionDelivery(transition()); await service.deliverDueAlerts(); } finally { rmSync(directory, { recursive: true, force: true }); }
  });
  ...
  it('retries bounded provider failures and records ambiguous delivery', async () => {
    ...
    const sender = vi.fn(async () => { throw new Error('network timeout'); });
    const service = createAlertService({ store, botToken: 'secret', recipientIds: ['ops-a'], timeoutMs: 5000, now: () => new Date('2026-08-23T00:00:00Z'), sender });
    try {
      await service.queueTransitionDelivery(transition());
      await service.deliverDueAlerts(new Date('2026-08-23T00:00:00Z'));
      expect(store.readDashboardOverview().recentDeliveries[0]).toMatchObject({ state: 'delivery_ambiguous', attemptCount: 1, lastErrorCode: 'delivery_failed' });
    } finally { rmSync(directory, { recursive: true, force: true }); }
  });
});
