...
export function formatAlertText(input: { level: 'warning' | 'critical'; monitor: string; occurrenceCount: number; observedAt: string; recovered: boolean }): string {
  const localTime = new Intl.DateTimeFormat('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh', dateStyle: 'short', timeStyle: 'medium' }).format(new Date(input.observedAt));
  return [
...
export function createAlertService(deps: AlertServiceDeps) {
  ...
  async function deliverDueAlerts(at: Date = now(), limit = 50): Promise<void> {
    ...
    for (const delivery of deliveries) {
      const incident = delivery.incidentId ? deps.store.getIncident(delivery.incidentId) : undefined;
      const text = delivery.kind === 'collector_failed'
        ? 'CRITICAL: ops-collector stopped; open https://man.thienuy.edu.vn'
        : formatAlertText({ level: incident?.level ?? 'critical', monitor: incident?.monitor ?? 'collector', occurrenceCount: incident?.occurrenceCount ?? delivery.attemptCount, observedAt: incident?.lastSeenAt ?? at.toISOString(), recovered: delivery.kind === 'recovered' });
      try {
        await sender({ botToken: deps.botToken, recipientId: delivery.recipientId, timeoutMs: deps.timeoutMs }, text);
...
export async function sendCollectorFailureNotice(config: Omit<ZaloSendConfig, 'recipientId'> & { recipientIds: string[] }, fetchImpl?: typeof fetch): Promise<void> {
  const text = 'CRITICAL: ops-collector stopped; open https://man.thienuy.edu.vn';
  for (const recipientId of config.recipientIds) await sendZaloText({ ...config, recipientId, fetchImpl }, text);
}
