...
describe('Beszel v0.18.8 contract', () => {
  it('pins the runtime and strips fields outside the public contract', () => {
    ...
    expect(systemStatsListSchema.parse(fixture('system-stats')).items[0]).not.toHaveProperty('unknownField');
    expect(systemdServicesListSchema.parse(fixture('systemd-services')).items[0]).not.toHaveProperty('cpuPeak');
  });

  it('rejects a different Hub version and out-of-range service enums', () => {
    expect(() => hubInfoSchema.parse({ v: '0.19.0', cu: false })).toThrow();
    expect(() => systemdServicesListSchema.parse({
