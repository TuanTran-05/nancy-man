...
import { systemRecordSchema, systemStatsListSchema, systemdServicesListSchema, hubInfoSchema } from './contracts.js';
...
describe('Beszel telemetry normalization', () => {
  ...
  it('uses null for missing optional arrays and rejects invalid/future metric timestamps', () => {
    ...
    value.stats = systemStatsListSchema.parse({
      ...
      items: [{ created: '2026-08-24T02:40:45.000Z', stats: { cpu: 1, m: 1, mu: 1, mp: 1, mb: 0, s: 0, su: 0, d: 1, du: 1, dp: 1 } }],
    }).items[0];
    const normalized = normalizeBeszelSnapshot(value, new Date('2026-08-24T02:41:00.000Z'));
    expect(normalized.resources).toMatchObject({ cpuUserPercent: null, load1: null, networkReceiveBytesPerSecond: null, diskReadBytesPerSecond: null, swapPercent: null });

    const future = raw();
    future.stats = { ...future.stats, created: '2026-08-24T02:42:00.000Z' };
    expect(() => normalizeBeszelSnapshot(future, new Date('2026-08-24T02:41:00.000Z'))).toThrow('beszel_contract_invalid');
  });
  ...
  it('drops stale services, sorts failed first and caps the display list', () => {
    ...
    value.services = {
      ...value.services,
      totalItems: 3,
      items: value.services.items.map((item, index) => ({ ...item, name: index === 0 ? 'nginx' : index === 1 ? 'postgresql' : 'unapproved-unit', updated: index === 2 ? 1787530000000 : 1787539245000 })),
    };
    const normalized = normalizeBeszelSnapshot(value, new Date('2026-08-24T02:41:00.000Z'));
    expect(normalized.services.map(({ name }) => name)).toEqual(['postgresql', 'nginx']);

    const tooShort = { ...raw(), services: { ...raw().services, items: raw().services.items.slice(0, 2) } };
    expect(() => normalizeBeszelSnapshot(tooShort, new Date('2026-08-24T02:41:00.000Z'))).toThrow('beszel_contract_invalid');
  });
});
