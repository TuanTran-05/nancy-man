...
export function createBeszelClient(config: Extract<BeszelCollectorConfig, { enabled: true }>, deps: ClientDeps = {}) {
  ...
  const fetchResponse = async (url: string, init: RequestInit, signal: AbortSignal): Promise<Response> => {
    try {
  ...
  const authenticate = async (signal: AbortSignal): Promise<string> => {
    ...
    const response = await fetchResponse(makeUrl(FIXED_PATHS.auth), {
      method: 'POST',
    ...
    if (!parsed.success || parsed.data.record.email !== config.username) throw boundedError('beszel_contract_invalid');
    cachedToken = parsed.data.token;
    return cachedToken;
  };

  let reauthUsed = false;
  const protectedJson = async (path: string, params: Record<string, string> | undefined, signal: AbortSignal, allowReauth: boolean): Promise<unknown> => {
    const token = cachedToken ?? await authenticate(signal);
    const response = await fetchResponse(makeUrl(path, params), {
  ...
  return {
    async readSnapshot(): Promise<BeszelRawSnapshot> {
      ...
      try {
        ...
        const systemRaw = await protectedJson(`${FIXED_PATHS.systems}${config.systemId}`, undefined, signal, true);
        const statsRaw = await protectedJson(FIXED_PATHS.stats, {
          page: '1',
          perPage: '1',
          sort: '-created',
        ...
        const servicesRaw = await protectedJson(FIXED_PATHS.services, {
          page: '1',
          perPage: '200',
        ...
        if (!hub.success || !system.success || !stats.success || !services.success) throw boundedError('beszel_contract_invalid');
        if (system.data.id !== config.systemId || services.data.totalItems > services.data.items.length) throw boundedError('beszel_contract_invalid');
