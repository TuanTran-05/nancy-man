...
function makeFetch(options: { info401?: boolean; second401?: boolean } = {}) {
  ...
  let second401 = options.second401 ?? false;
  const fetchImpl = vi.fn(async (input: RequestInfo | URL, init: RequestInit = {}) => {
    const url = String(input);
    calls.push({ url, init });
    const path = new URL(url).pathname;
    if (path.endsWith('/auth-with-password')) return response(fixture('auth'));
    if (path === '/api/beszel/info' && info401) { info401 = false; return response({ error: 'expired' }, 401); }
...
describe('Beszel fixed endpoint client', () => {
  it('authenticates once, caches the token and calls only the pinned endpoints', async () => {
    ...
    const client = createBeszelClient(config, { fetchImpl: fake.fetchImpl as typeof fetch, readPassword: () => 'fixture-password\n' });
    ...
    expect(JSON.parse(String(fake.calls[0].init.body))).toEqual({ identity: config.username, password: 'fixture-password' });
    expect(fake.calls.slice(1).every(({ init }) => (init.headers as Record<string, string>).Authorization === 'fixture-token-not-a-secret')).toBe(true);
  ...
  it('reauthenticates once after 401 and rejects a second 401', async () => {
    ...
    const client = createBeszelClient(config, { fetchImpl: first.fetchImpl as typeof fetch, readPassword: () => 'fixture-password' });
    await client.readSnapshot();
    expect(first.calls.filter(({ url }) => new URL(url).pathname.endsWith('/auth-with-password'))).toHaveLength(2);

    const second = makeFetch({ info401: true, second401: true });
    const failedClient = createBeszelClient(config, { fetchImpl: second.fetchImpl as typeof fetch, readPassword: () => 'fixture-password' });
    await expect(failedClient.readSnapshot()).rejects.toMatchObject({ code: 'beszel_auth_failed' });
    expect(second.calls.filter(({ url }) => new URL(url).pathname.endsWith('/auth-with-password'))).toHaveLength(2);
  });

  it('fails closed for identity mismatch and truncated service pages', async () => {
    const mismatch = makeFetch();
    mismatch.fetchImpl.mockImplementation(async (input: RequestInfo | URL, init: RequestInit = {}) => {
      mismatch.calls.push({ url: String(input), init });
      const path = new URL(String(input)).pathname;
      if (path.endsWith('/auth-with-password')) return response({ ...(fixture('auth') as object), record: { ...(fixture('auth') as { record: object }).record, email: 'other@example.invalid' } });
      return response(fixture(path === '/api/beszel/info' ? 'info' : path.includes('/systems/') ? 'system' : path.endsWith('/system_stats/records') ? 'system-stats' : 'systemd-services'));
    });
    await expect(createBeszelClient(config, { fetchImpl: mismatch.fetchImpl as typeof fetch, readPassword: () => 'fixture-password' }).readSnapshot()).rejects.toMatchObject({ code: 'beszel_contract_invalid' });

    const truncated = makeFetch();
    truncated.fetchImpl.mockImplementation(async (input: RequestInfo | URL, init: RequestInit = {}) => {
      truncated.calls.push({ url: String(input), init });
      const path = new URL(String(input)).pathname;
      if (path.endsWith('/auth-with-password')) return response(fixture('auth'));
      if (path.endsWith('/systemd_services/records')) return response({ ...(fixture('systemd-services') as object), totalItems: 201 });
      return response(fixture(path === '/api/beszel/info' ? 'info' : path.includes('/systems/') ? 'system' : 'system-stats'));
    });
    await expect(createBeszelClient(config, { fetchImpl: truncated.fetchImpl as typeof fetch, readPassword: () => 'fixture-password' }).readSnapshot()).rejects.toMatchObject({ code: 'beszel_contract_invalid' });
  });
  ...
  it('maps timeout, network, HTTP, JSON, schema and empty-stat failures to bounded codes', async () => {
    const cases: Array<[string, (url: string, init: RequestInit) => Promise<Response>, string]> = [
      ['network', async () => { throw new TypeError('password=fixture-password token=fixture-token'); }, 'beszel_unreachable'],
      ...
      ['empty', async (url) => url.includes('/system_stats/') ? response({ page: 1, perPage: 1, totalPages: 1, totalItems: 0, items: [] }) : response(fixture(new URL(url).pathname === '/api/beszel/info' ? 'info' : new URL(url).pathname.includes('/systems/') ? 'system' : new URL(url).pathname.includes('/systemd_services/') ? 'systemd-services' : 'auth')), 'beszel_no_stats'],
    ];
    for (const [_name, implementation, expected] of cases) {
      const fetchImpl = vi.fn(implementation);
      const error = await createBeszelClient(config, { fetchImpl: fetchImpl as typeof fetch, readPassword: () => 'fixture-password' }).readSnapshot().catch((value: unknown) => value as BeszelClientError);
  ...
  it('uses one deadline for the complete snapshot', async () => {
    const fetchImpl = vi.fn(async (_url: string, init: RequestInit) => await new Promise<Response>((_resolve, reject) => {
