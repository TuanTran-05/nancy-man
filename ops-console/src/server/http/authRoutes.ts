...
const credentialsSchema = z.object({ username: z.string().min(1).max(64), password: z.string().min(1).max(256), totp: z.string().regex(/^\d{6}$/u) }).strict();
...
export function attachAuthRoutes(router: Router, auth: AuthService): void {
  router.post('/api/session', async (request, response) => {
    ...
    if (!parsed.success) { response.status(401).json({ error: 'Invalid credentials' }); return; }
    try {
      const session = await auth.authenticate(parsed.data);
      response.setHeader('Set-Cookie', `__Host-ops_session=${session.token}; Max-Age=28800; Path=/; Secure; HttpOnly; SameSite=Strict`);
      response.status(201).json({ username: session.username, csrfToken: session.csrfToken, expiresAt: session.expiresAt });
    } catch {
  ...
  router.get('/api/session', requireOpsSession(auth), (request: SessionRequest, response) => {
    noStore(response);
    response.json({ username: request.opsSession!.username, csrfToken: request.opsSession!.csrfToken, expiresAt: request.opsSession!.expiresAt });
  });
  ...
  router.delete('/api/session', requireOpsSession(auth), (request: SessionRequest, response) => {
    ...
    response.setHeader('Set-Cookie', '__Host-ops_session=; Max-Age=0; Path=/; Secure; HttpOnly; SameSite=Strict');
