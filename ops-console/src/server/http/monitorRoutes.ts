...
import type { DashboardOverview, InfrastructureHistoryRange, MonitorSample } from '../../shared/models.js';
...
const resourceKeys = ['cpuPercent', 'cpuUserPercent', 'cpuSystemPercent', 'cpuIoWaitPercent', 'memoryPercent', 'memoryUsedBytes', 'memoryTotalBytes', 'swapPercent', 'swapUsedBytes', 'swapTotalBytes', 'load1', 'load5', 'load15', 'cpuThreads', 'uptimeSeconds', 'diskPercent', 'diskUsedBytes', 'diskTotalBytes', 'diskReadBytesPerSecond', 'diskWriteBytesPerSecond', 'diskIoUtilizationPercent', 'networkReceiveBytesPerSecond', 'networkTransmitBytesPerSecond', 'agentVersion', 'metricObservedAt', 'probeOk'] as const;
...
function publicSample(sample: MonitorSample): MonitorSample {
  const publicHostResources = (details: Record<string, unknown>): Record<string, unknown> => Object.fromEntries(resourceKeys.map((key) => [key, key === 'agentVersion' || key === 'metricObservedAt' ? (typeof details[key] === 'string' ? details[key] : null) : key === 'probeOk' ? details[key] === true : (typeof details[key] === 'number' && Number.isFinite(details[key]) ? details[key] : null)]));
  ...
  const publicHostServices = (details: Record<string, unknown>): Record<string, unknown> => ({
    matchedTotal: typeof details.matchedTotal === 'number' && Number.isFinite(details.matchedTotal) ? details.matchedTotal : null,
    failedServices: Array.isArray(details.failedServices) ? details.failedServices.filter((value): value is string => typeof value === 'string').slice(0, 32) : [],
    services: Array.isArray(details.services) ? details.services.slice(0, 32).flatMap((value) => {
      if (!value || typeof value !== 'object' || Array.isArray(value)) return [];
      const item = value as Record<string, unknown>;
      if (typeof item.name !== 'string' || typeof item.state !== 'string' || typeof item.subState !== 'string' || typeof item.observedAt !== 'string') return [];
      return [{ name: item.name, state: item.state, subState: item.subState, cpuPercent: typeof item.cpuPercent === 'number' && Number.isFinite(item.cpuPercent) ? item.cpuPercent : null, memoryBytes: typeof item.memoryBytes === 'number' && Number.isFinite(item.memoryBytes) ? item.memoryBytes : null, observedAt: item.observedAt }];
    }) : [],
  });
  const details = sample.monitor === 'host_resources'
    ...
    : sample.monitor === 'host_services'
      ...
      : sample.monitor === 'beszel'
        ...
        : Object.fromEntries(Object.entries(sample.details).filter(([key]) => publicDetailKeys.has(key)).map(([key, value]) => [key, safeValue(value)]));
...
export function attachMonitorRoutes(router: Router, store: OpsStore, auth: AuthService): void {
  ...
  router.get('/api/infrastructure/history', guard, (request, response) => {
    ...
    if (!range.success) { response.status(400).json({ error: 'invalid_range' }); return; }
    const ranges: Record<InfrastructureHistoryRange, { milliseconds: number; resolutionSeconds: 60 | 300 | 1800 | 7200 }> = {
    ...
    const to = new Date();
    const points = store.readInfrastructureHistory({ from: new Date(to.getTime() - selected.milliseconds).toISOString(), to: to.toISOString(), resolutionSeconds: selected.resolutionSeconds, limit: 720 });
    response.json({ range: range.data, resolutionSeconds: selected.resolutionSeconds, collectedAt: to.toISOString(), points });
  });
