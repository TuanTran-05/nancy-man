...
import type { OpsStore, AccountRecord, SessionRecord } from '../storage/store.js';
import { decryptSecret, encryptSecret, createOpaqueToken, hashPassword, hashToken, verifyPassword } from './crypto.js';
...
export function provisionAccount(
  ...
  if (!/^[a-zA-Z0-9._-]{3,64}$/.test(username)) throw new Error('Username must contain only safe characters');
  ...
  store.recordAuditEvent({ actorId: null, action: 'account_provisioned', target: id, details: { username }, occurredAt: createdAt });
  return {
...
export function createAuthService(deps: AuthDependencies) {
  ...
  function createSession(account: Pick<AccountRecord, 'id' | 'username'>): AuthSession {
    ...
    return { token, csrfToken, accountId: account.id, username: account.username, expiresAt, absoluteExpiresAt };
  }

  async function authenticate(input: { username: string; password: string; totp: string }): Promise<AuthSession> {
    ...
    deps.store.recordLoginAttempt({ username, attemptedAt: attemptedAt.toISOString(), success: valid });
    if (!valid || !account) {
      deps.store.recordAuditEvent({ actorId: account?.id ?? null, action: 'login_failed', target: username.slice(0, 64), details: { reason: 'invalid_credentials' }, occurredAt: attemptedAt.toISOString() });
      throw new InvalidCredentialsError();
    }
    deps.store.recordAuditEvent({ actorId: account.id, action: 'login_succeeded', target: account.id, details: {}, occurredAt: attemptedAt.toISOString() });
  ...
  function requireSession(token: string): RequiredSession {
    ...
    const current = now();
    if (!session || Date.parse(session.expiresAt) <= current.getTime() || Date.parse(session.absoluteExpiresAt) <= current.getTime()) {
...
export const createSession = (service: ReturnType<typeof createAuthService>, account: Pick<AccountRecord, 'id' | 'username'>) => service.createSession(account);
export const authenticate = (service: ReturnType<typeof createAuthService>, input: { username: string; password: string; totp: string }) => service.authenticate(input);
export const requireSession = (service: ReturnType<typeof createAuthService>, token: string) => service.requireSession(token);
export const destroySession = (service: ReturnType<typeof createAuthService>, token: string) => service.destroySession(token);
