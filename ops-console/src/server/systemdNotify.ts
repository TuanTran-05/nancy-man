...
const normalizeSocketPath = (socketPath: string): string => socketPath.startsWith('@') ? `\0${socketPath.slice(1)}` : socketPath;
...
export function startSystemdWatchdog(env: Environment = process.env, sender: NotifySender = sendSystemdNotification): { stop: () => void } {
  ...
  const safeSend = (message: string): void => {
    try { sender(message, socketPath); } catch { /* Notification failure must not stop collection. */ }
  };
  safeSend('READY=1');
  ...
  const interval = setInterval(() => safeSend('WATCHDOG=1'), Math.max(1000, Math.floor(watchdogUsec / 2000)));
  interval.unref?.();
