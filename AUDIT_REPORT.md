...
- Level manager scoped by grade range with safe default (deny on unknown level)
...
- `useLevelManagementData.ts` fetches ALL students/evaluations/sessions without server-side filtering (performance + security)
...
- `useLevelManagementData.ts` fetching all students/evaluations without pagination
...
| Classes     | Admin                                      | Teacher (own), Accounting, Level Manager |
...
| 10  | Move `useLevelManagementData` to server-side reads               | 4h     | `src/hooks/useLevelManagementData.ts`, `api/read/[channel].ts`                                                     |
...
- [ ] **All roles tested** -- admin, teacher, accounting, level_manager, student, parent
