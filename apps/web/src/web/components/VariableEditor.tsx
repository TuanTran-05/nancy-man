...
export function variableEditPolicy(
  item: Pick<VariableInventoryItem, 'mutability' | 'requirement'>
) {
  ...
  return {
    ...
    reason:
      item.mutability === 'observed'
        ? 'Observed definitions cannot be changed.'
        : item.requirement === 'unknown'
          ? 'Unknown definitions must be cataloged first.'
          : undefined
  };
}
    ...
    <div className="variable-editor">
      {editing ? (
        <>
          ...
          <button type="button" onClick={() => onStage(value)}>
            Đưa vào bản nháp
          </button>
        </>
      ) : (
      ...
      {policy.deletable ? (
        <button
          type="button"
          onClick={() => {
            if (window.confirm('Xóa giá trị tùy chọn khỏi source?')) onDelete();
          }}
        >
          Xóa tùy chọn
        </button>
