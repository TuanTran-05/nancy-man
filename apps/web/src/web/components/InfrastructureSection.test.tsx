...
const history = (
  range: InfrastructureHistoryResponse['range'] = '24h'
): InfrastructureHistoryResponse => ({
  range,
  resolutionSeconds: range === '24h' ? 300 : 1800,
...
const makeOverview = (
  ...
  latestByMonitor: {
    beszel: {
      ...
      observedAt,
      latencyMs: null,
      details: { probeOk: !stale, hubVersion: '0.18.8', systemStatus: stale ? 'down' : 'up' },
      errorCode: stale ? 'beszel_unavailable' : null
    },
    host_resources: {
      ...
      details: stale
        ...
        : ({
            ...
            load5: 0.8,
            diskPercent: 50,
            diskUsedBytes: 24 * 1024 ** 3,
            diskTotalBytes: 48 * 1024 ** 3,
            diskReadBytesPerSecond: 1048576,
            diskWriteBytesPerSecond: 2097152,
...
describe('InfrastructureSection', () => {
  it('renders current cards, four charts, accessible ranges and failed services first', async () => {
    const loadHistory = vi.fn(async (range: InfrastructureHistoryResponse['range']) =>
      history(range)
    );
    render(
      <InfrastructureSection
        overview={makeOverview()}
        now={Date.parse('2026-08-24T00:01:00Z')}
  ...
  it('hides stale current values, keeps history visible and handles failed/null history', async () => {
    ...
    const first = render(
      <InfrastructureSection
        overview={makeOverview('2026-08-23T23:58:29Z', true)}
        now={Date.parse('2026-08-24T00:01:00Z')}
        loadHistory={loadHistory}
      />
    );
