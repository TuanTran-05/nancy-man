...
describe('Beszel local fake-server integration', () => {
  it('round-trips the exact query and one 401 retry without Internet', async () => {
    ...
    const server = createServer(async (request, response) => {
      ...
      if (path === '/api/beszel/info' && forceUnauthorized) {
        forceUnauthorized = false;
        return send(response, { error: 'expired' }, 401);
      }
      ...
      if (path.endsWith('/systemd_services/records'))
        return send(response, fixture('systemd-services'));
    ...
    try {
      const snapshot = await createBeszelClient(config, {
        fetchImpl,
      ...
      expect(
        requests.every(
          ({ path, authorization }) =>
            path.endsWith('/auth-with-password') || authorization === 'fixture-token-not-a-secret'
        )
      ).toBe(true);
      ...
      expect(new URL(stats, origin).searchParams.get('filter')).toBe(
        'system="abc123def456ghi" && type="1m"'
      );
    } finally {
