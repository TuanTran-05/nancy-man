...
const makeRaw = (now = '2026-08-24T02:41:00.000Z'): BeszelRawSnapshot => ({
  ...
  stats: systemStatsListSchema.parse({
    ...
    items: [
      {
        ...systemStatsListSchema.parse(fixture('system-stats')).items[0],
        created: new Date(Date.parse(now) - 10_000).toISOString()
      }
    ]
  }).items[0],
  services: systemdServicesListSchema.parse({
    ...(fixture('systemd-services') as object),
    items: systemdServicesListSchema
      .parse(fixture('systemd-services'))
      .items.map((item) => ({ ...item, updated: Date.parse(now) - 10_000 }))
  })
});
...
describe('Beszel probe', () => {
  it('returns the three normalized monitors with safe details on success', async () => {
    ...
    const samples = await createBeszelProbe({
      readSnapshot: async () => makeRaw(now.toISOString())
    })(now);
    expect(samples.map(({ monitor }) => monitor)).toEqual([
      'beszel',
      'host_resources',
      'host_services'
    ]);
    expect(samples[0]).toMatchObject({
      level: 'healthy',
      errorCode: null,
      details: { probeOk: true, hubVersion: '0.18.8', systemStatus: 'up' }
    });
  ...
  it('returns only a bounded failure sample for every adapter error', async () => {
    ...
    for (const code of codes) {
      const samples = await createBeszelProbe({
        readSnapshot: async () => {
          throw new BeszelClientError(code);
        }
  ...
  it('does not publish stale or down system metrics as current samples', async () => {
    ...
    expect(await createBeszelProbe({ readSnapshot: async () => down })(now)).toEqual([
      expect.objectContaining({ errorCode: 'beszel_agent_down' })
    ]);
    const stale = makeRaw(new Date(now.getTime() - 181_000).toISOString());
    expect(await createBeszelProbe({ readSnapshot: async () => stale })(now)).toEqual([
      expect.objectContaining({ errorCode: 'beszel_metric_stale' })
    ]);
  });
});
