...
const subStateMap: readonly InfrastructureServiceSubState[] = [
  'dead',
  'running',
  'exited',
  'failed',
...
function parseMetricTimestamp(value: string, now: Date): string {
  ...
  if (!Number.isFinite(timestamp) || timestamp - now.getTime() > 5000)
    throw new BeszelClientError('beszel_contract_invalid');
  return new Date(timestamp).toISOString();
}

export function normalizeBeszelSnapshot(
  raw: BeszelRawSnapshot,
  now = new Date()
): NormalizedBeszelSnapshot {
  ...
  if (expectedServiceTotal !== rawServices.length || raw.services.totalItems > rawServices.length)
    throw new BeszelClientError('beszel_contract_invalid');
  ...
  const failedServices = freshServices
    .filter((item) => item.state === 'failed' || item.subState === 'failed')
    .map((item) => item.name)
