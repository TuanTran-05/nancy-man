...
describe('PostgreSQL probe', () => {
  ...
  it('runs only the fixed aggregate query and validates its exact response', async () => {
      ...
      probePostgres(
        {
          ...
          clientFactory: () => ({
            query: async (sql: string) => {
              queries.push(sql);
              return { rows: [{ snapshot: validSnapshot }] };
            },
            end: async () => undefined
          })
        },
    ...
    ).resolves.toMatchObject({ monitor: 'postgres', level: 'healthy', details: validSnapshot });
  ...
  it('maps malformed metric responses without exposing raw database data', async () => {
    await expect(
      probePostgres(
        {
          postgresUrl: 'postgres://unused',
          clientFactory: () => ({
            query: async () => ({ rows: [{ snapshot: { query: 'secret' } }] }),
            end: async () => undefined
          })
        },
        now
      ...
      level: 'critical',
      errorCode: 'database_invalid_metric',
      details: {}
    });
  });
});
