...
const responseSchema = z
  .object({
    ok: z.boolean(),
    result: z.object({ message_id: z.union([z.string(), z.number()]) }).optional(),
...
export async function sendZaloText(
  config: ZaloSendConfig,
  text: string
): Promise<{ messageId: string }> {
  if (!config.botToken || !config.recipientId)
    throw new ZaloDeliveryError('invalid_configuration', false);
  if (text.length < 1 || text.length > 2000)
    throw new ZaloDeliveryError('message_length_invalid', false);
  ...
  try {
    ...
    try {
      response = await (config.fetchImpl ?? fetch)(
        `https://bot-api.zaloplatforms.com/bot${config.botToken}/sendMessage`,
        {
          method: 'POST',
    ...
    if (!response.ok) {
      ...
      if (/invalid[_ -]?recipient|chat[_ -]?not[_ -]?found/iu.test(errorCode))
        throw new ZaloDeliveryError('invalid_recipient', false);
      if ([403, 408, 429].includes(response.status) || response.status >= 500)
        throw new ZaloDeliveryError(`provider_http_${response.status}`, true);
