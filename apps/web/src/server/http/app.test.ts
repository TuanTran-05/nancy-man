...
const makeZaloFixture = () => {
  ...
  const app = createOpsApp({
    ...
    zalo: {
      ...
      now: () => now,
      confirmationSender: async (config, text) => {
        sent.push({ recipientId: config.recipientId, text });
...
describe('protected Ops HTTP API', () => {
  it('denies monitoring API without an Ops session', async () => {
    ...
    try {
      await request(fixture.app).get('/api/overview').expect(401);
    } finally {
      fixture.cleanup();
    }
  });
  ...
  it('logs in with password plus TOTP and sets a secure host-only cookie', async () => {
    ...
    try {
      ...
      expect(response.headers['set-cookie'][0]).toMatch(
        /__Host-ops_session=.*Secure.*HttpOnly.*SameSite=Strict/
      );
  ...
  it('rejects an acknowledge request without the session CSRF token', async () => {
    const fixture = makeFixture();
    try {
      const incident = fixture.store.upsertIncident({
      ...
      await request(fixture.app)
        .post(`/api/incidents/${incident.id}/ack`)
        .set('Cookie', cookie)
        .send({ note: 'Đã xem' })
        .expect(403);
      await request(fixture.app)
  ...
  it('links only through the separate Ops bot webhook and keeps replay idempotent', async () => {
    ...
    try {
      ...
      await request(fixture.app)
        ...
        .set('Cookie', cookie)
        .expect(200)
        .expect(({ body }) => expect(body).toEqual({ linked: false }));
      await request(fixture.app)
        .post('/api/zalo/link-code')
        .set('Cookie', cookie)
        .send({})
        .expect(403);
      const codeResponse = await request(fixture.app)
        .post('/api/zalo/link-code')
        .set('Cookie', cookie)
        .set('X-CSRF-Token', login.body.csrfToken)
        .send({})
      ...
      await request(fixture.app)
        ...
        .set('X-Bot-Api-Secret-Token', 'wrong')
        .send({})
        .expect(403);
      const payload = {
        ...
        message: {
          ...
          text: codeResponse.body.command,
      ...
      await request(fixture.app)
        ...
        .set('Cookie', cookie)
        .expect(200)
        .expect(({ body }) => expect(body.linked).toBe(true));
      expect(fixture.sent).toEqual([
        { recipientId: 'chat-123', text: expect.stringContaining('Đã liên kết') }
      ]);
    } finally {
      fixture.cleanup();
    }
  ...
  it('serves authenticated bounded history ranges with no-store headers', async () => {
    ...
    try {
      ...
      for (const [range, resolution] of [
        ...
        const response = await request(fixture.app)
          .get(`/api/infrastructure/history?range=${range}`)
          .set('Cookie', cookie)
          .expect(200);
      ...
      await request(fixture.app)
        .get('/api/infrastructure/history?range=invalid')
        .set('Cookie', cookie)
        .expect(400)
        .expect({ error: 'invalid_range' });
      await request(fixture.app)
        ...
        .expect({ error: 'invalid_range' });
      await request(fixture.app)
        .get('/api/infrastructure/history?range=1h&range=24h')
        .set('Cookie', cookie)
        .expect(400)
        .expect({ error: 'invalid_range' });
  ...
  it('projects new monitor details without passing raw credentials or nested service fields', async () => {
    ...
    try {
      fixture.store.recordSample({
        monitor: 'beszel',
        level: 'healthy',
        observedAt: '2026-08-24T00:00:45.000Z',
        latencyMs: null,
        details: {
          probeOk: true,
          hubVersion: '0.18.8',
      ...
      fixture.store.recordSample({
        ...
        observedAt: '2026-08-24T00:00:45.000Z',
        latencyMs: null,
        details: { cpuPercent: 42.5, token: 'fixture-token', host: 'secret-host' },
        errorCode: null
      });
      fixture.store.recordSample({
      ...
      const response = await request(fixture.app)
        ...
        .set('Cookie', cookie)
        .expect(200);
      expect(response.body.latestByMonitor.host_services.details.services[0]).toEqual({
        name: 'nginx',
        state: 'active',
        subState: 'running',
        cpuPercent: 0.3,
      ...
      expect(JSON.stringify(response.body)).not.toMatch(
        /fixture-token|fixture-password|command|environment|description|restartCount|raw|secret-host/
      );
    } finally {
      fixture.cleanup();
    }
