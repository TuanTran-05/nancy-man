...
const fixture = (name: string): unknown =>
  JSON.parse(
    readFileSync(new URL(`../server/beszel/fixtures/${name}.json`, import.meta.url), 'utf8')
  );
...
const config: CollectorConfig = {
  nodeEnv: 'test',
  dbPath: '/tmp/ops.sqlite',
  appUrl: 'http://127.0.0.1:3000',
  postgresUrl: 'postgres://unused',
  pm2PidPath: '/tmp/pm2.pid',
  pm2ErrorLogPath: '/tmp/pm2.log',
  cronLogPath: '/tmp/cron.log',
...
describe('Beszel contract smoke', () => {
  it('returns only safe aggregate fields', async () => {
    ...
    const result = await smokeBeszelContract(
      config,
    ...
    expect(output).not.toMatch(
      /abc123def456ghi|ops-telemetry|fixture-token|fixture-public-key|private-sensor|host|name|key|token|password|services|stats/i
    );
  ...
  it('rejects disabled config without reading or printing credentials', async () => {
    await expect(smokeBeszelContract({ ...config, beszel: { enabled: false } })).rejects.toThrow(
      'beszel_smoke_requires_enabled_config'
    );
  });
