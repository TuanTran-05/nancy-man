...
let blockedExternalContacts = 0;
globalThis.fetch = async (input, init) => {
  const requested = new URL(
    typeof input === 'string' || input instanceof URL ? String(input) : input.url,
    parsedBaseUrl.origin
  );
  if (requested.origin !== parsedBaseUrl.origin) {
    blockedExternalContacts += 1;
    throw new Error('OPS_E2E_EXTERNAL_NETWORK_BLOCKED');
  }
  return originalFetch(input, init);
};

const passwordSalt = randomBytes(16);
const passwordKey = scryptSync(password, passwordSalt, 64, {
  N: 16384,
  r: 8,
  p: 1,
  maxmem: 64 * 1024 * 1024
...
server.once('error', () => {
  console.error('OPS_E2E_FIXTURE_BIND_FAILED');
  rmSync(directory, { recursive: true, force: true });
  process.exitCode = 1;
});
server.once('listening', () => {
  console.log(
    'OPS_E2E_FIXTURE_READY base_url=' +
      parsedBaseUrl.origin +
      ' sqlite=temporary external_network=blocked'
  );
});
...
).run(accountId, 'ops-e2e', passwordHash, encryptedSeed, new Date().toISOString());
db.prepare(
  'INSERT INTO monitor_samples (monitor, level, observed_at, latency_ms, details_json, error_code) VALUES (?, ?, ?, ?, ?, ?)'
).run(
  'app_liveness',
  'healthy',
  new Date().toISOString(),
  4,
  JSON.stringify({ probeOk: true, release: 'e2e' }),
  null
);
...
const resourceDetails = {
  ...
  cpuThreads: 4,
  probeOk: true,
  metricObservedAt: telemetryObservedAt
};
const serviceDetails = {
  matchedTotal: 2,
  failedServices: ['edutrack-worker'],
  services: [
    {
...
db.prepare(
  'INSERT INTO monitor_samples (monitor, level, observed_at, latency_ms, details_json, error_code) VALUES (?, ?, ?, ?, ?, ?)'
).run('host_resources', 'healthy', telemetryObservedAt, 8, JSON.stringify(resourceDetails), null);
db.prepare(
  'INSERT INTO monitor_samples (monitor, level, observed_at, latency_ms, details_json, error_code) VALUES (?, ?, ?, ?, ?, ?)'
).run(
  'host_services',
...
db.prepare(
  'INSERT INTO incidents (id, dedupe_key, monitor, level, state, occurrence_count, opened_at, last_seen_at, recovered_at, acknowledged_at, acknowledged_by, note, safe_summary) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
).run(
  'e2e-incident',
  'app_liveness:e2e',
  'app_liveness',
  ...
  null,
  null,
  'Synthetic E2E incident'
);
db.close();

const close = () => {
  server.close(() => {
    console.log('OPS_E2E_SERVER_CONTACTS escaped=0 blocked=' + blockedExternalContacts);
    rmSync(directory, { recursive: true, force: true });
    process.exit(0);
  });
};
