...
describe('classifySeverity', () => {
  ...
  it('marks financial/core multi-user failures high and isolated retryable failures medium', () => {
    ...
    expect(
      classifySeverity({ source: 'browser', errorCode: 'STUDENT_LOAD_FAILED', retryable: true })
    ).toBe('medium');
    expect(
      classifySeverity({ source: 'browser', errorCode: 'OPTIONAL_WIDGET_FAILED', handled: true })
    ).toBe('low');
  });
});
