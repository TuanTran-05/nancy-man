...
export function suggestEndDate({
  ...
  const selectedDays = daysOfWeek?.length ? daysOfWeek : [getDateOnlyDayOfWeek(startDate)];

  return calculateEndDate(startDate, getRequiredSessions(grade), selectedDays, holidays);
}

function getDateOnlyDayOfWeek(dateStr: string): number {
