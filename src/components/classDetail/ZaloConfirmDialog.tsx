...
interface ZaloConfirmDialogProps {
  zaloConfirmData: { studentId: string; date: string; classId: string } | null;
  students: Student[];
...
export const ZaloConfirmDialog: React.FC<ZaloConfirmDialogProps> = ({
  zaloConfirmData,
  students,
  className,
  isSendingZalo,
  ...
  const { t } = useLanguage();
  useBodyScrollLock(!!zaloConfirmData);
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-white">
                          {t.zaloConfirmDialog.title}
                        </h3>
                        <p className="text-sm text-blue-100">{t.zaloConfirmDialog.description}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 space-y-4">
                    <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 space-y-2">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t.zaloConfirmDialog.classLabel}
                        </span>
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t.zaloConfirmDialog.date}
                        </span>
                        <span className="font-medium text-slate-700 dark:text-slate-300">
                          {formatVN(zaloConfirmData.date, 'dd/MM/yyyy')}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t.zaloConfirmDialog.parentPhone}
                        </span>
                    ...
                    <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400">
                        {t.zaloConfirmDialog.znsWarning}
                      </p>
                    </div>
                  </div>
                    ...
                    >
                      {isSendingZalo ? (
                        <>
                          ...
                          {t.zaloConfirmDialog.sending}
                        </>
                      ) : (
                        <>
                          <Send className="w-4 h-4" /> {t.zaloConfirmDialog.sendNow}
                        </>
                      )}
                    </button>
                  </div>
