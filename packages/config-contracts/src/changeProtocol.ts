...
const value = z
  .string()
  .max(65_536)
  .refine((item) => !item.includes('\u0000'), 'Value contains NUL');
...
export const ChangeItemSchema = itemMetadata
  .extend({
  ...
  .superRefine((item, context) => {
    ...
    if (item.operation === 'delete' && item.value !== undefined) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['value'],
        message: 'Delete cannot contain a value'
      });
    }
    if (item.mutability === 'observed' || item.requirement === 'unknown') {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['mutability'],
        message: 'Definition is not editable'
      });
    }
    if (item.operation === 'delete' && item.requirement !== 'optional') {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['operation'],
        message: 'Only optional definitions may be deleted'
      });
    }
  });
  ...
  items.forEach((item, index) => {
    ...
    if (seen.has(key)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['items', index],
        message: 'Duplicate source/catalog item'
      });
    }
    seen.add(key);
  });
  ...
  schema.superRefine((body, context) => {
    ...
    if (items && items.some((item) => item.appId !== body.appId)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['items'],
        message: 'A change may target one application only'
      });
    }
  });
...
export const ChangeValidateRequestSchema = oneApplication(
  z
    ...
    .strict()
    .superRefine((body, context) => uniqueItemIds(body.items, context))
);
export type ChangeValidateRequest = z.infer<typeof ChangeValidateRequestSchema>;

export const ChangeImpactPlanSchema = z
  .object({
...
export const ChangeSaveRequestSchema = z
  .object({
    changeId,
    changeDigest,
    catalogVersion: version,
...
export const ChangeApplyRequestSchema = z
  .object({
    changeId,
    runId,
    changeDigest,
...
export const ChangeStatusRequestSchema = z
  .object({ changeId, afterEventId: eventId.optional() })
  .strict();
export type ChangeStatusRequest = z.infer<typeof ChangeStatusRequestSchema>;
export const ClearApplyBlockRequestSchema = z
  .object({
    appId: stableId,
  ...
  .strict()
  .refine((body) => body.appId === body.confirmationAppId, {
    path: ['confirmationAppId'],
    message: 'Application confirmation does not match'
  });
export type ClearApplyBlockRequest = z.infer<typeof ClearApplyBlockRequestSchema>;

export const ChangeValidationResponseSchema = z
  .object({
    changeId,
    state: ConfigChangeStateSchema,
    changeDigest,
    itemFingerprints: z.array(
      z
        .object({
          catalogId: stableId,
          sourceId: stableId,
          oldValueFingerprint: sourceFingerprint.nullable(),
          newValueFingerprint: sourceFingerprint.nullable()
        })
        .strict()
    ),
...
export const ChangeSavedResponseSchema = z
  .object({
    changeId,
  ...
  .strict();
export const ChangeApplyStartedResponseSchema = z
  .object({
    changeId,
    runId,
  ...
  .strict();
export const ChangeCancelledResponseSchema = z
  .object({
    changeId,
    state: z.literal('CANCELLED')
  })
  .strict();
export const ApplyBlockClearedResponseSchema = z
  .object({
    appId: stableId,
...
export const ChangeStatusEventSchema = z
  .object({
    eventId,
    changeId,
...
export const ChangeStatusResponseSchema = z
  .object({
    changeId,
    state: ConfigChangeStateSchema,
